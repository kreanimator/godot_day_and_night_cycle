...
@export_range( 0.0, 1.0, 0.0001 ) var time_scale : float = 0.01: #This value is optimal to the game
	set( value ) :
		time_scale = value
...
func _process( delta: float ) -> void :
	if not Engine.is_editor_hint() : # We don't want a time lapse in the editor
		...
		#DEBUG
		#$Label.text = "Day time " + str(day_time)
...
func _update_clouds() -> void :
	if is_instance_valid( environment ) :
		self.environment.sky.sky_material.set_shader_parameter( "clouds_cutoff", clouds_cutoff )
		self.environment.sky.sky_material.set_shader_parameter( "clouds_weight", clouds_weight )

func _update_shader() -> void :
	if is_instance_valid( environment ) :
		self.environment.sky.sky_material.set_shader_parameter(
			"overwritten_time",
